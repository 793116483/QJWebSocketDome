...
+(instancetype)messageWithSenderId:(NSString *)senderId displayName:(NSString *)displayName text:(NSString *)text date:(NSDate*)date isCurrentUser:(BOOL)isCurrentUser
{
    ...
    // font             : 头像字的字体大小
    // diameter         : 头像显示成圆形直径
    message.msgAvatarImage = [JSQMessagesAvatarImageFactory avatarImageWithUserInitials:displayName backgroundColor:[UIColor blueColor] textColor:[UIColor whiteColor] font:[UIFont systemFontOfSize:13] diameter:50];
